...
class M3uParser:
    ...
    def parse_m3u(self, path: str, check_live: bool = True, enforce_schema: bool = True):
        ...
        if is_valid_url(path):
            logging.debug("Started parsing m3u link...")
            try:
            ...
            logging.debug("Started parsing m3u file...")
            try:
    ...
    def to_file(self, filename: str, format: str = "json"):
        ...
        if len(self._streams_info) == 0:
            logging.debug("Either parsing is not done or no stream info was found after parsing !!!")
            return
        ...
        if format == "json":
            ...
            with open(filename, mode="w", encoding="utf-8") as fp:
                fp.write(data)
            logging.debug("Saved to file: %s" % filename)
            ...
            if self._enforce_schema:
                ...
                logging.warning("Saving to csv file not supported if the schema was not forced (enforce_schema) !!!")
